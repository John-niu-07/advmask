...
model.load_state_dict(torch.load('/art/model_cifar/untargeted/model_pgd_untargeted_cifar.model'))
...
sample_adv_img = (sample_adv[0]*255).astype(np.uint8).transpose(1, 2, 0)
...
rec_sample_adv_img = (rec_sample_adv.reshape((3,32,32))*255).astype(np.uint8).transpose(1, 2, 0)
...
input_img = (input_sample.reshape((3,32,32))*255).astype(np.uint8).transpose(1, 2, 0)
...
#model3.load_state_dict(torch.load('/art/model_cifar/ae/model_ae_dim16.model'))
#model3.load_state_dict(torch.load('/art/model_cifar/ae/model_ae_dim128_100_.model'))
#model3.load_state_dict(torch.load('/art/model_cifar/ae/model_ae_dim128_100_101.model'))
#model3.load_state_dict(torch.load('/art/model_cifar/ae/model_ae_dim128_mse.model'))
#model3.load_state_dict(torch.load('/art/model_cifar/ae/model_ae_dim128_mse_.model'))
model3.load_state_dict(torch.load('/art/model_cifar/ae/model_ae_dim128_mse__.model'))
...
for i in range(1):
    ...
    f_name = "/art/vis_cifar/ae/ae100_image_"+str(i)+".png"
...
#classifier.fit_ae(x_train, y_train, batch_size=256, nb_epochs=402)
...
#classifier.save('model_ae_dim16','/art/model_cifar/ae/')
#classifier.save('model_ae_dim128_100_','/art/model_cifar/ae/')
#classifier.save('model_ae_dim128_100_101','/art/model_cifar/ae/')
#classifier.save('model_ae_dim128_mse','/art/model_cifar/ae/')
#classifier.save('model_ae_dim128_mse_','/art/model_cifar/ae/')
#classifier.save('model_ae_dim128_mse__','/art/model_cifar/ae/')
...
for i in range(5):
    rec_x = classifier.predict(x_train[i])
    rec_img = rec_x.reshape((3, 32, 32))
    #accuracy = np.sum(np.argmax(predictions, axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
    #print("Accuracy on benign test examples: {}%".format(accuracy * 100))
    img_ = (x_train[i]*255).astype(np.uint8).transpose(1, 2, 0)
    rec_img_ = (rec_img*255).astype(np.uint8).transpose(1, 2, 0)
    
    f_name = "/art/vis_cifar/ae/input_image_"+str(i)+".png"
    save_image(img_, f_name)
    f_name = "/art/vis_cifar/ae/rec_ae100_image_"+str(i)+".png"
    save_image(rec_img_, f_name)
