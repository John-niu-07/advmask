...
from art.preactresnet import PreActBlock, PreActBottleneck, PreActResNet, PreActResNet18, initialize_weights, PreActResNet18STL, PreActResNet34STL, PreActResNet50STL, PreActResNet34
...
'''
print('vis feat for tiny without M')
x_test, y_test = x_test[:500], y_test[:500]
np.save('y_tiny_woM.npy',y_test)
'''
...
for idx in range(len(x_train)):
    ...
    x = x_train_[idx].transpose(1,2,0)
    x = transform0(x)
...
transform2 = transforms.Compose(
            ...
            [transforms.ToTensor(), transforms.RandomCrop(64, padding=4), transforms.RandomHorizontalFlip()]
            #[transforms.ToTensor()]
            )
...
class ResNetSimCLR_ft(nn.Module):
    ...
    def forward(self, x):
        ...
        #np.save('x'+str(self.kk)+'_tiny_woM_sp_eps4.npy',x_save)
        #np.save('x'+str(self.kk)+'_tiny_wM_sp_eps4.npy',x_save)
        #np.save('x'+str(self.kk)+'_tiny_woM_sp_eps8.npy',x_save)
        np.save('x'+str(self.kk)+'_tiny_wM_sp_eps8.npy',x_save)
        self.kk = self.kk + 1
        '''
...
if 1:
    #model_ft = torch.load('/art/model_tiny/clr/model_tiny_wM_sp_eps4.model')
    #model_ft = torch.load('/art/model_tiny/clr/model_tiny_wM_sp_eps8.model')
    model_ft = torch.load('/art/model_tiny/clr/model_tiny64_wM_sp_eps4.model')
    modelD.load_state_dict(model_ft)
else:

    ###load
    #model_ft = torch.load('/art/model_tiny/clr/model_tiny_advftM_sp_pre34.model')
    #model_ft = torch.load('/art/model_tiny/clr/model_tiny_advftM_sp.model')
    model_ft = torch.load('/art/model_tiny/clr/model_tiny64__advftM_sp.model')
...
if 0:
    ...
    print('----------vis-----------------')
    #model_ft = torch.load('/art/model_tiny/clr/model_tiny_woM_sp_eps4.model')
    #model_ft = torch.load('/art/model_tiny/clr/model_tiny_woM_sp_eps8.model')
    model_ft = torch.load('/art/model_tiny/clr/model_tiny64__woM_sp_eps4.model')
...
#classifier.fit_predict(x_train, y_train, x_test, y_test, batch_size=32, nb_epochs=51)
...
#190
#classifier.save('model_tiny_advftM_sp','/art/model_tiny/clr/')
#classifier.save('model_tiny_advftM_sp_pre34','/art/model_tiny/clr/')

#classifier.save('model_tiny_woM_sp_eps4','/art/model_tiny/clr/')
#classifier.save('model_tiny_woM_sp_eps8','/art/model_tiny/clr/')
#classifier.save('model_tiny64__woM_sp_eps4','/art/model_tiny/clr/')
...
#np.save('x_test_adv_tiny64_sp_eps4.npy', x_test_adv)
x_test_adv = np.load('x_test_adv_tiny64_sp_eps4.npy')

#np.save('x_test_adv_tiny_sp.npy', x_test_adv)
#x_test_adv = np.load('x_test_adv_tiny_sp.npy')

#np.save('x_test_adv_tiny_sp_eps8.npy', x_test_adv)
#x_test_adv = np.load('x_test_adv_tiny_sp_eps8.npy')
...
trainer = AdversarialTrainerMadryPGD(classifierD, nb_epochs=40, batch_size=64, eps= 4.0 /255.0, eps_step = 2.0/255.0)
...
#classifierD.save('model_tiny_wM_sp','/art/model_tiny/clr/')
#classifierD.save('model_tiny_wM_sp_eps8','/art/model_tiny/clr/')
#classifierD.save('model_tiny64_wM_sp_eps4','/art/model_tiny/clr/')


'''
print('vis feat for tiny with M .........')
for k in range(0, 15):
    print(k)
    x_t = x_test[k*32:k*32+32]
...
#np.save('x_test_adv2_tiny64_sp_eps4.npy', x_test_adv2)
x_test_adv2 = np.load('x_test_adv2_tiny64_sp_eps4.npy')

#np.save('x_test_adv2_tiny_sp.npy', x_test_adv2)
#x_test_adv2 = np.load('x_test_adv2_tiny_sp.npy')

#np.save('x_test_adv2_tiny_sp_eps8.npy', x_test_adv2)
#x_test_adv2 = np.load('x_test_adv2_tiny_sp_eps8.npy')
