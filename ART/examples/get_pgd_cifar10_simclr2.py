...
from art.vgg import vgg11, vgg16, vgg16_bn
from art.shufflenetv2 import shufflenet_v2_x0_5,  shufflenet_v2_x1_5
from art.mobilenet import mobilenet_v2
...
#lod = torch.load('/art/SimCLR2/pytorch-simclr-simclr-master/checkpoint/tiny32_vgg16.pth')
#lod = torch.load('/art/SimCLR2/pytorch-simclr-simclr-master/checkpoint/tiny32_shuffle.pth')
#lod = torch.load('/art/SimCLR2/pytorch-simclr-simclr-master/checkpoint/tiny32_shuffle_v2.pth')
#lod = torch.load('/art/SimCLR2/pytorch-simclr-simclr-master/checkpoint/tiny32_mobile.pth')
...
class ResNetSimCLR_ft(nn.Module):
    ...
    def __init__(self, base_model, out_dim):
        ...
        #self.backbone.linear =  nn.Linear(1280, 10)

    def forward(self, x):
        x = self.backbone(x)

        #x, x_vis = self.backbone(x)
        #print(x.shape)
        #print(x_vis.shape)
        #x_vis = x_vis[0][0].detach().cpu().numpy()
        #print(x_vis)
        #pert_img = (x_vis*255).astype(np.uint8)
        #save_image(pert_img, "./feat_vis_.png")
...
#opt = torch.optim.SGD(model.parameters(), lr=0.0005, momentum=0.9, weight_decay=5e-4)
...
#classifier.fit_predict(x_train, y_train, x_test, y_test, batch_size=2, nb_epochs=1)

#feas_numpy = ((activation[str(visual_layer)].squeeze().cpu()) * 255).numpy()
#for i in range(0, feas_numpy.shape[0]):
#    plt.imsave('./_{}.png'.format(i), feas_numpy[i], cmap = plt.cm.gray)
