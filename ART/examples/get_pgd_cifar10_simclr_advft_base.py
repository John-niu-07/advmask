...
classifierAdvFt = PyTorchClassifier(
            model=model,
            clip_values=(-3.0, 3.0),
...
#classifier.fit_predict(x_train_adv, y_train, x_test, y_test, batch_size=64, nb_epochs=51)
#predictions = classifier.predict(x_test)
#predictions2 = classifier.predict(x_test_adv)

classifierAdvFt.fit_predict(x_train_adv, y_train, x_test, y_test, batch_size=64, nb_epochs=81)
predictions = classifierAdvFt.predict(x_test)
predictions2 = classifierAdvFt.predict(x_test_adv)
...
predictions = classifierAdvFt.predict(x_test_adv2)
accuracy = np.sum(np.argmax(predictions, axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
