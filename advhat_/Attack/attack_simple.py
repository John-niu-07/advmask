...
def main(args):
        ...
        logo = tf.placeholder(tf.float32,shape=[None,400,500,3],name='logo_input')
        ...
        mask = np.zeros((1,600,600,3))
        mask[0, 50:450, 50:550, :] = np.ones((400, 500, 3))

        #logo_img = np.zeros((1,600,600,3))
        #logo_img[0, 50:100, 200:400, :] = logo[:, :, :, :]
        logo_img = tf.pad(logo, [[0,0],[50,150],[50,50],[0,0]])
        ...
        #final_crop = tf.clip_by_value(stn(united, theta, (400,400)), 0., 1.)
        ...
        init_logo = np.ones((400,500,3))*127./255.
        if args.init_logo!=None:
                init_logo[:] = io.imread(args.init_logo)/255.
        init_logo_old = np.ones((400,500,3))*127./255.

        #logo_img = np.zeros((600,600,3))
        #logo_img[50:100, 200:400 :] = init_logo
        #fdict[logo] = np.expand_dims(logo_img,0)
        ...
        #print(img_out.shape)
        io.imsave('aaa.png', img_out[0])

        #'''
        img_hat = sess.run(hat,feed_dict=fdict)
        #print(img_hat.shape)
        hat_out = np.zeros((400,500,3))
        hat_out = img_hat[0, 50:450, 50:550, : ]
        io.imsave('hat_out2.png', hat_out)
        #'''
        ...
        if args.anchor_face!=None:
                ...
                '''
                anch_im = io.imread(args.anchor_face)/255.
                fdict[face_input] = np.expand_dims(anch_im,0)
                fdict2[image_input] = prep(sess.run(final_crop_orig, feed_dict=fdict))
                fdict2[orig_emb] = sess.run(embedding,feed_dict=fdict2)
                '''
                ...
                fdict2[orig_emb] = sess.run(embedding,feed_dict=fdict2)
                '''
                anch_im = io.imread(args.image)/255.
                fdict[face_input] = np.expand_dims(anch_im,0)
                fdict2[image_input] = prep(sess.run(final_crop_orig, feed_dict=fdict))
                fdict2[orig_emb] = sess.run(embedding,feed_dict=fdict2)
                '''
        ...
        while True:
                ...
                print(moments[3,3:5,:])
                print(init_logo[3,3:5,:])
                init_logo -= step_val*np.sign(moments)
                #init_logo += step_val*np.sign(moments)
                init_logo = np.clip(init_logo,0.,1.)
                print(init_logo[3,3:5,:])

                diff = np.mean(np.abs(init_logo - init_logo_old))
                print(diff)
                init_logo_old = init_logo.copy()
