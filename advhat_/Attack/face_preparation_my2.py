...
def main(args):
	...
	if args.mask:
		logo_mask = np.ones((1,900,900,3),dtype=np.float32)

		logo = tf.placeholder(tf.float32,shape=[1,900,900,3])
		param = tf.placeholder(tf.float32,shape=[1,1])
		ph = tf.placeholder(tf.float32,shape=[1,1])
		result = projector2(param,ph,logo)
		...
		#'''
		#logo_input = (io.imread('/face/hat/advhat/Attack/john_hatcrop2.png')/255.).reshape((1,900,900,3))
		logo_input = (io.imread('/face/hat/advhat/Attack/john_hat_hatimg2_900.jpg')/255.).reshape((1,900,900,3))
		logo2 = tf.placeholder(tf.float32,shape=[1,900,900,3])
		result2 = projector2(param,ph,logo2)
		...
		#io.imsave(args.image[:-4]+'_mask2.png',img_with_mask)
		#io.imsave(args.image[:-4]+'_hatimg2.png',img_hat_crop)
		io.imsave(args.image[:-4]+'_withhat2.png',img_with_hat)
...
def parse_arguments(argv):
    ...
    parser.add_argument('--y', type=float, default=50., help='Translation of the sticker along y-axis')
