...
import utils_nio
import losses
from config_nio import patch_config_types
from nn_modules import LandmarkExtractor, FaceXZooProjector, TotalVariation
from utils_nio import load_embedder, EarlyStopping, get_patch
...
class AdversarialMask:
    def __init__(self, config):
        ...
        self.train_no_aug_loader, self.train_loader = utils_nio.get_train_loaders(self.config)

        self.embedders = load_embedder(self.config.train_embedder_names, device)

        face_landmark_detector = utils_nio.get_landmark_detector(self.config, device)
        ...
        self.create_folders()
        utils_nio.save_class_to_file(self.config, self.config.current_dir)
        self.target_embedding = utils_nio.get_person_embedding(self.config, self.train_no_aug_loader, self.config.celeb_lab_mapper, self.location_extractor,
                                                           self.fxz_projector, self.embedders, device)
        self.best_patch = None
        print(self.config.celeb_lab_mapper)

        self.blue_mask_t = utils_nio.load_mask(self.config, self.config.blue_mask_path, device)
    ...
    def train(self):
        adv_patch_cpu = utils_nio.get_patch(self.config)
        ...
        utils_nio.plot_train_val_loss(self.config, self.train_losses_epoch, 'Epoch')
        utils_nio.plot_train_val_loss(self.config, self.train_losses_iter, 'Iterations')
        utils_nio.plot_separate_loss(self.config, self.train_losses_epoch, self.dist_losses, self.tv_losses, self.tv_losses, self.tv_losses)
