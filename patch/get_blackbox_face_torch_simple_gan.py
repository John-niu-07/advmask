...
class Net(nn.Module):
    def __init__(self, config):
        ...
        self.mask = transforms.ToTensor()(Image.open("/face/Mask/AdversarialMask/datasets/012_mask6.png").convert('RGB')).to(device).unsqueeze(0)	
...
#img_target = transforms.ToTensor()(Image.open("/face/Mask/AdversarialMask/datasets/dancing.jpg").convert('RGB')).to(device).unsqueeze(0)
#img_target = transforms.ToTensor()(Image.open("/face/Mask/AdversarialMask/datasets/building.jpg").convert('RGB')).to(device).unsqueeze(0)
#img_target = F.interpolate(img_target, (112, 112))
img_target = transforms.Resize([112,112])(img_target)
...
mask = transforms.ToTensor()(Image.open("/face/Mask/AdversarialMask/datasets/012_mask6.png").convert('RGB')).to(device).unsqueeze(0)	
