...
class Net(nn.Module):
    def __init__(self, config):
        ...
        img2 = transforms.ToTensor()(Image.open("/face/Mask/AdversarialMask/datasets/CASIA/1302735/001_aligned.png").convert('RGB')).to(device).unsqueeze(0)	
        img2 = F.interpolate(img2, (112, 112))
        for emb_name, emb_model in self.embedders.items():
            self.img2_feat = emb_model(img2.to(device)).detach()

			
        self.mask = transforms.ToTensor()(Image.open("/face/Mask/AdversarialMask/datasets/012_mask2.png").convert('RGB')).to(device).unsqueeze(0)	
        #self.mask = transforms.ToTensor()(Image.open("/face/Mask/AdversarialMask/datasets/012_mask4.png").convert('RGB')).to(device).unsqueeze(0)	
        self.mask = F.interpolate(self.mask, (112, 112))

        self.img_input = transforms.ToTensor()(Image.open("/face/Mask/AdversarialMask/datasets/CASIA/1302735/001_aligned.png").convert('RGB')).to(device).unsqueeze(0)	
...
mask = transforms.ToTensor()(Image.open("/face/Mask/AdversarialMask/datasets/012_mask2.png").convert('RGB')).to(device).unsqueeze(0)	
#mask = transforms.ToTensor()(Image.open("/face/Mask/AdversarialMask/datasets/012_mask4.png").convert('RGB')).to(device).unsqueeze(0)	
...
img_input = transforms.ToTensor()(Image.open("/face/Mask/AdversarialMask/datasets/CASIA/1302735/001_aligned.png").convert('RGB')).to(device).unsqueeze(0)
...
new_size = tuple(config.magnification_ratio * s for s in config.img_size)
transforms.Resize(new_size)(mask_print)
