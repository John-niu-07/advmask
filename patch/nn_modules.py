...
class FaceXZooProjector(nn.Module):
    def __init__(self, device, img_size, patch_size):
        ...
        self.uv_mask_src_mask = transforms.ToTensor()(Image.open('../prnet/new_uv6.png').convert('L')).to(device).unsqueeze(0)
        self.uv_mask_src_logo = transforms.ToTensor()(Image.open('../prnet/new_uv6_.png'))[0:3,:,:].to(device).unsqueeze(0)
    ...
    def forward(self, img_batch, landmarks, adv_patch, uv_mask_src=None, do_aug=False, is_3d=False):
        ...
        #print('---')
        #img_batch = F.interpolate(img_batch, (256, 256))
        #print(img_batch.shape)
        #print(pos_orig[:, 0].shape)
        #print(pos_orig[:, 1].shape)
        #print(self.uv_face_src.shape)
        #io.imsave('s1.png',np.transpose(img_batch[0].cpu(),[1,2,0]))
        ...
        if not is_3d:
            ...
            self.uv_mask_src_mask = self.uv_mask_src_mask.repeat(adv_patch.shape[0], 1, 1, 1)
            if adv_patch.shape[2] != 256:
                adv_patch = F.interpolate(adv_patch, (256, 256))
                uv_mask_src = F.interpolate(uv_mask_src, (256, 256))
                self.uv_mask_src_mask = F.interpolate(self.uv_mask_src_mask, (256, 256))
                self.uv_mask_src_logo = F.interpolate(self.uv_mask_src_logo, (256, 256))
            texture_patch = adv_patch

        if do_aug:
            texture_patch, uv_mask_src = self.augment_patch(texture_patch, uv_mask_src)

        # without logo or with logo: for_testing
        new_texture = texture_patch * uv_mask_src + texture_img * (1 - uv_mask_src) #for testing


        # with logo: for_training
        ##new_texture = texture_patch * self.uv_mask_src_mask + texture_img * (1 - uv_mask_src) #black logo
        #new_texture = texture_patch * self.uv_mask_src_mask + self.uv_mask_src_logo  + texture_img * (1 - uv_mask_src) 
...
class FaceXZooProjector_align(nn.Module):
    def __init__(self, device, img_size, patch_size):
        super(FaceXZooProjector_align, self).__init__()
        #self.prn = PRN('./prnet/prnet.pth', device, patch_size[0])
        self.prn = PRN('../prnet/prnet.pth', device)
