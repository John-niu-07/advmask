...
import utilsD_opencvalign_large_contrastive_stnbb as utils
import losses
from config_ganD_opencvalign_large_contrastive_stnbb import patch_config_types
from nn_modules import LandmarkExtractor, FaceXZooProjector, TotalVariation
from utilsD_opencvalign_large import load_embedder, EarlyStopping, get_patch
...
loader = transforms.Compose([
    transforms.Resize([250,250]),  # scale imported image
...
class Net(nn.Module):
    ...
    def stn(self, x):
        ...
        grid = F.affine_grid(theta, x.size()).to(device)
...
class AdversarialMask:
    def __init__(self, config):
        ...
        self.contrastive_embedding = utils.get_person_embedding_z_contrastive(self.config, self.train_no_aug_loader, self.config.celeb_lab_mapper, self.location_extractor,
                                                           self.fxz_projector, self.embedders, device)
        ...
        self.patch_mask = F.interpolate(self.patch_mask, (250, 250))        
    ...
    def train(self):
        ...
        if 1:
            ...
            latent_z_np = np.load('./init_face3_opencvalign_112.npy').reshape((1, 14, 512))
            latent_z_np = np.load('./init_face3_opencvalign_088.npy').reshape((1, 14, 512))
            latent_z_np = np.load('./init_face3_oldalign_088.npy').reshape((1, 14, 512))
            latent_z_np = np.load('./init_zhu_000.npy').reshape((1, 14, 512))

            latent_z_np = np.load('./init_lh_oldalign_085.npy').reshape((1, 14, 512))
            latent_z_np = np.load('./init_yyj_oldalign_058.npy').reshape((1, 14, 512))
            ...
            control_z_np = np.load('./target_face.npy').reshape((1, 14, 512))
        ...
        for epoch in range(self.config.epochs):
            ...
            for i_batch, (img_batch, img_name, cls_id) in progress_bar:
                #print('   ')
                #print(cls_id)
                ...
                if cls_id == target_id:
                    ...
                    #print(cls_id)
                    #print(img_name)
                    #io.imsave(str(cls_id)+'.png', np.transpose(img_batch[0],[1,2,0]))
                    continue
                ...
                cnt += 1
                (b_loss, sep_loss), vars = self.forward_step(img_batch, latent_z, cls_id, epoch, contrastive_ids)
    ...
    def loss_fn(self, patch_embs, D_loss, cls_id, latent_z, adv_patch, epoch, contrastive_ids):
        ...
        distance_contrastive_loss = torch.empty(0, device=device)
        #print('     ')
        for contrastive_id in contrastive_ids:
            #print(contrastive_id)
            con_id = torch.ones(1, dtype=torch.int64) * contrastive_id #target cls_id=5
            con_id = con_id.to(device)
            for target_embedding, (emb_name, patch_emb) in zip(self.target_embedding.values(), patch_embs.items()):

                target_embeddings = torch.index_select(target_embedding, index=con_id, dim=0).squeeze(-2)
                #print(target_embeddings.shape)
                #print(target_embeddings[0][0:5])
                distance_ = self.dist_loss(patch_emb, target_embeddings)
                single_embedder_dist_loss_ = torch.mean(distance_).unsqueeze(0)
                distance_contrastive_loss = torch.cat([distance_contrastive_loss, single_embedder_dist_loss_], dim=0)
        distance_contrastive_loss = self.config.dist_weight * distance_contrastive_loss.mean()
        #print(distance_contrastive_loss)
        ...
        if stable >8:
            ...
            total_loss += 10*torch.clamp( torch.norm( latent_z - self.latent_z_init, p=2 ) - 160, min=0)
            latent_loss = torch.norm( latent_z - self.latent_z_init, p=2 ) 


            neg_loss=0
            for tp_z in self.latent_neg_z:
                neg_loss += 20*torch.clamp( self.rad -torch.norm(latent_z - tp_z, p=2), min=0)
            total_loss += neg_loss
            print(neg_loss)
            latent_loss2 = neg_loss 
        '''
        ...
        #total_loss += 10 * distance_contrastive_loss
        total_loss += 0.01 * distance_contrastive_loss


        #total_loss += 100*torch.clamp( torch.norm( latent_z - self.latent_z_init, p=2 ) - 160, min=0)
        latent_loss = torch.norm( latent_z - self.latent_z_init, p=2 )

        latent_loss2 = distance_contrastive_loss 


        gray = torch.sum(torch.abs(adv_patch[0,0,:,:]-adv_patch[0,1,:,:])) +  torch.sum(torch.abs(adv_patch[0,0,:,:]-adv_patch[0,2,:,:])) +  torch.sum(torch.abs(adv_patch[0,2,:,:]-adv_patch[0,1,:,:]))
        #print(gray)
        total_loss += gray * 0.01
    ...
    def forward_step(self, img_batch, latent_z, cls_id, epoch, contrastive_ids):
        img_batch = img_batch.to(device)
        ...
        if 0: #zoom in 
            ...
            adv_patch = F.interpolate(x_inv, (250, 250))
        ...
        alpha = np.random.uniform(-2.,2., size=(batch,1))/180.*np.pi

        theta = scale * np.hstack([np.cos(alpha), np.sin(alpha), -x1/250.,  -np.sin(alpha), np.cos(alpha), -y1/250.])
        self.stn.theta = torch.Tensor(theta)


        #img_batch_applied = img_batch * (1 - self.patch_mask) + self.stn(adv_patch) * self.patch_mask

        img_batch_applied = img_batch * (1 - self.stn(self.patch_mask)) + self.stn(adv_patch * self.patch_mask)
        ...
        for embedder_name, emb_model in self.embedders.items():
            img_batch_applied = F.interpolate(img_batch_applied, (112, 112))
        ...
        D_loss = pred_inv[0][0]
        loss = self.loss_fn(patch_embs, D_loss, cls_id, latent_z, adv_patch, epoch, contrastive_ids)
    ...
    def save_final_objects(self):
        ...
        if 0: #zoom in
            ...
            final_patch = F.interpolate(final_patch, (250, 250))
        ...
        final_patch = final_patch * self.patch_mask + torch.ones(3,250,250) * (1 - self.patch_mask)
