...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, patch, epoch):
        ...
        final_patch = torch.cat([patch.squeeze(0), self.alpha.squeeze(0)])
    ...
    def save_checkpoint_z(self, val_loss, patch, epoch, latent_z_init, latent_z_mask):
        """
        Saves model when validation loss decrease.
        """
        if self.verbose:
    ...
    def save_checkpoint_z2(self, val_loss, patch, epoch, latent_z_init, latent_z_mask):
        """
        Saves model when validation loss decrease.
        """
        if self.verbose:
...
@torch.no_grad()
def get_person_embedding_z(config, loader, celeb_lab, location_extractor, fxz_projector, embedders, device, include_others=False):
    ...
    for embedder_name, embedder in embedders.items():
        ...
        for img_batch, _, person_indices in tqdm(loader):
            ...
            if include_others:
                ...
                #patch_mask = transforms.ToTensor()(Image.open('/face/Mask/AdversarialMask/datasets/012_mask8.png').convert('L')).to(device).unsqueeze(0)
...
def get_patch(config):
    ...
    elif config.initial_patch == 'grey':
        patch = torch.ones((1, 3, config.patch_size[0], config.patch_size[1]), dtype=torch.float32) * 0.5        
    elif config.initial_patch == 'init':
        #patch_img = Image.open('/face/Mask/AdversarialMask/patch/23-09-2022_22-29-12/final_results/final_patch.png').convert('RGB')
        #patch = torch.from_numpy(np.expand_dims(np.transpose(np.array(patch_img), [2,0,1]),0)/255.).float()

        patch_img = transforms.ToTensor()(Image.open("/face/Mask/AdversarialMask/prnet/blue_init.png").convert('RGB')).unsqueeze(0)
        patch = F.interpolate(patch_img, (112, 112))
    print('get_patch')
    ...
    #with logo:
    #uv_face = transforms.ToTensor()(Image.open('../prnet/new_uv6.png').convert('L'))
    #uv_logo = transforms.ToTensor()(Image.open('../prnet/new_uv6_.png'))[0:3,:,:]
    #patch = patch * uv_face + uv_logo
...
def plot_separate_loss(config, train_losses_epoch, dist_losses, tv_losses, tv_losses2, tv_losses3):
    epochs = [x + 1 for x in range(len(train_losses_epoch))]
    #weights = np.array([config.dist_weight, config.tv_weight])
    weights = np.array([config.dist_weight, config.tv_weight, config.tv_weight, config.tv_weight])
    ...
    for weight, train_loss, label in zip(weights, [dist_losses,  tv_losses, tv_losses2, tv_losses3], ['Distance loss', 'Total Variation loss', 'latent 2', 'D_loss']):
        if weight > 0:
            ...
            axes[idx, 0].plot(epochs, train_loss, c='b', label='Train')
            axes[idx, 0].set_xlabel('Epoch')
            axes[idx, 0].set_ylabel('Loss')
            axes[idx, 0].set_title(label)
            axes[idx, 0].legend(loc='upper right')
            axes[idx, 0].xaxis.set_major_locator(MaxNLocator(integer=True))
            idx += 1
    fig.tight_layout()
