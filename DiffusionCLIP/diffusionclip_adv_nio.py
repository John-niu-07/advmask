...
class Net(nn.Module):
    ...
    def stn(self, x):
        ...
        grid = F.affine_grid(theta, x.size()).to('cuda')
...
class DiffusionCLIP(object):
    def __init__(self, args, config, device=None):
        ...
        self.target = None
        #self.mask = transforms.ToTensor()(Image.open('/face/Mask/DiffusionCLIP/012_mask6.png').convert('L')).to(device).unsqueeze(0)
        #self.mask = transforms.ToTensor()(Image.open('/face/Mask/DiffusionCLIP/012_mask9.png').convert('L')).to(device).unsqueeze(0)
        self.mask = transforms.ToTensor()(Image.open('/face/Mask/DiffusionCLIP/012_mask5.png').convert('L')).to(device).unsqueeze(0)
        self.mask = transforms.ToTensor()(Image.open('/face/Mask/DiffusionCLIP/012_mask_clip.png').convert('L')).to(device).unsqueeze(0)

        self.theta = torch.Tensor([1., 0., 0., 0., 1., 0.])
        #self.stn = Net(self.theta).to('cuda')
    ...
    def clip_finetune(self):
        ...
        if self.config.data.dataset == "LSUN":
            ...
            pass
        elif self.config.data.dataset == "NIO":
            pass
        else:
            raise ValueError

        if self.config.data.dataset in ["CelebA_HQ", "LSUN", "NIO"]:
            model = DDPM(self.config)
            if self.args.model_path:
                print('--------')
                #init_ckpt = torch.load(self.args.model_path)
                init_ckpt = torch.load('/face/Mask/DiffusionCLIP/cele/celeba_hq.ckpt')
            else:
                ...
                if 0:
                    ...
                    print('-------- run --------')
                    init_ckpt = torch.load('/face/Mask/DiffusionCLIP/cele/light_weight.pth')
                    Flag_first = 0
            learn_sigma = False
        ...
        if Flag_first == 1:
            # add by yu ####### for loading our weights #####################
            if isinstance(init_ckpt, list):
                print('-------run once--------')
                model_dict = torch.load('/face/Mask/DiffusionCLIP/cele/celeba_hq.ckpt.bk')
                # model_dict = model.state_dict()
                for k, v in init_ckpt[0].items():
                    temp = k.split('.', 1)[-1]
                    v.requires_grad = True
                    if temp in model_dict:
                        model_dict[temp] = v
                    else:
                        print(f'{temp} is not in model')
                init_ckpt = model_dict


            save_dir = '/face/Mask/DiffusionCLIP/cele/light_weight.pth'
            torch.save(init_ckpt, save_dir)
            # add by yu ####### for loading our weights #####################
        ...
        for mode in ['train', 'test']:
            ...
            pairs_path = os.path.join('precomputed/',
                                      f'{self.config.data.category}_{mode}_t{self.args.t_0}_nim{self.args.n_precomp_img}_ninv{self.args.n_inv_step}_pairs_adv.pth')
            print(pairs_path)
            ...
            for step, img in enumerate(loader):
                ...
                with torch.no_grad():
                    ...
                    tvu.save_image((x_lat + 1) * 0.5, os.path.join(self.args.image_folder,
                                                                   f'{mode}_{step}_lat_ninv{self.args.n_inv_step}.png'))
                ...
                tvu.save_image((x + 1) * 0.5, os.path.join(self.args.image_folder,
                                                           f'{mode}_{step}_rec_ninv{self.args.n_inv_step}.png'))
                if step == self.args.n_precomp_img - 1:
                    break
        ...
        for src_txt, trg_txt in zip(self.src_txts, self.trg_txts):
            ...
            for it_out in range(self.args.n_iter):
                ...
                save_name = f'checkpoint/{exp_id}_{trg_txt.replace(" ", "_")}-{it_out}_adv.pth'
                if self.args.do_train:
                    if os.path.exists(save_name):
                        ...
                        for step, (x0, x_id, x_lat) in enumerate(img_lat_pairs_dic['train']):
                            ...
                            x = x * self.mask + x_id * (1-self.mask)
                            ...
                            x = x_id * (1 - self.stn(self.mask)) + self.stn(x * self.mask)
                            '''
                            ...
                            loss_id = torch.mean(id_loss_func(self.target, x))
                            loss_l1 = nn.L1Loss()(x0, x)
                            #loss = self.args.clip_loss_w * loss_clip + self.args.id_loss_w * loss_id + self.args.l1_loss_w * loss_l1
                            #loss = self.args.clip_loss_w * loss_clip + self.args.id_loss_w * 0.00005 * loss_id + self.args.l1_loss_w * loss_l1

                            gray = torch.sum(torch.abs(x[0,0,:,:]-x[0,1,:,:])) +  torch.sum(torch.abs(x[0,0,:,:]-x[0,2,:,:])) +  torch.sum(torch.abs(x[0,2,:,:]-x[0,1,:,:]))


                            #loss = 0.00005 * loss_id + 0.0001 * loss_l1 + 0.0000000001 * gray
                            loss = 0.00005 * loss_id + 0.0001 * loss_l1 + 0.00000001 * gray
                            #loss = self.args.clip_loss_w * loss_clip + self.args.id_loss_w * 0.001 * loss_id + self.args.l1_loss_w * loss_l1
                            loss.backward()

                            optim_ft.step()
                            print(f"CLIP {step}-{it_out}: loss_id: {loss_id:.3f}, loss_clip: {loss_clip:.3f}, loss_l1: {loss_l1:.3f}, gray: {gray:.3f}")

                            if self.args.save_train_image:
                                tvu.save_image((x + 1) * 0.5, os.path.join(self.args.image_folder,
                                                                           f'train_{step}_{trg_txt.replace(" ", "_")}_{it_out}_ngen{self.args.n_train_step}.png'))

                            '''
                            #if step == 1 and it_out==15 :
                            if step == 1 and it_out==14 :
                                tvu.save_image((x + 1) * 0.5, os.path.join('/sdiff/DiffPure/advimg/',
                                                                           f'advimg_{it_out}.png'))
                            '''
                ...
                if self.args.do_test:
                    ...
                    for step, (x0, x_id, x_lat) in enumerate(img_lat_pairs):
                        with torch.no_grad():
                            ...
                            tvu.save_image((x + 1) * 0.5, os.path.join(self.args.image_folder,
                                                                       f'{mode}_{step}_{trg_txt.replace(" ", "_")}_{it_out}_ngen{self.args.n_test_step}.png'))
