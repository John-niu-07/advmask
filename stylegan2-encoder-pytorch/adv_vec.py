...
#z_now = np.load('/face/Mask/AdversarialMask/patch/init_face.npy')
#z_now = np.load('/face/Mask/AdversarialMask/patch/init_face3_opencvalign_t.npy')
#z_now = np.load('/face/Mask/AdversarialMask/patch/init_yyj_oldalign_058.npy')

z_now = np.load('/face/Mask/AdversarialMask/patch/init_face000005.npy').reshape((1, 14, 512))

#z_now_ = torch.from_numpy(z_now).to(device)
...
with torch.no_grad():
    for k in range(10):
        ...
        #imshow(tensor2image(imgs_gen))
        print(imgs_gen.shape)
        img = tensor2image(imgs_gen)
        img = Image.fromarray((img*255.).astype(np.uint8))
        img.save('img_'+str(k)+'.png')


    '''
    z_hat = encoder(imgs_gen)
    imgs_hat, _ =  generator([z_hat], 
                           input_is_latent=True,
                           truncation=truncation,
                           truncation_latent=trunc,
                           randomize_noise=False)
...
#imgs_hats = torch.cat([img_hat for img_hat in imgs_hat], dim=1)


print('initial projections:')
imshow(tensor2image(torch.cat([imgs_real, imgs_fakes], dim=2)),10)
#imshow(tensor2image(torch.cat([imgs_hats, imgs_fakes], dim=2)),10)
'''
